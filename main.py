...
from service import (code_generation_token_pipeline_stream, code_fix_token_pipeline_stream)
import json
...
async def run_generation_token_stream(req: PipelineRequest):
    async def event_generator():
        async for item in code_generation_token_pipeline_stream(req.model_id, req.prompt):
            # 기존 스트리밍 엔드포인트와 동일하게 줄바꿈으로 청크 구분
            yield json.dumps(item) + "\n"
    return StreamingResponse(event_generator(), media_type="application/json")
...
async def run_fix_token_stream(req: FixRequest):
    async def event_generator():
        async for item in code_fix_token_pipeline_stream(req.code, req.analysis):
            yield json.dumps(item) + "\n"
    return StreamingResponse(event_generator(), media_type="application/json")
